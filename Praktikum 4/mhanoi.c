...
void putarMatriksKecil(Matrix *matriksKecil) {
    int pojokKiriAtas = ELMT(*matriksKecil, 0, 0);
    int pojokKananAtas = ELMT(*matriksKecil, 0, 1);
    int pojokKiriBawah = ELMT(*matriksKecil, 1, 0);
    int pojokKananBawah = ELMT(*matriksKecil, 1, 1);

    ELMT(*matriksKecil, 0, 0) = pojokKiriBawah;
    ELMT(*matriksKecil, 0, 1) = pojokKiriAtas;
    ELMT(*matriksKecil, 1, 0) = pojokKananBawah;
    ELMT(*matriksKecil, 1, 1) = pojokKananAtas;
}
...
int main(void) {
    ...
    Matrix main, duakalidua;
    readMatrix(&main, N, N);

    int barisMatriks = ROW_EFF(main);
    int kolomMatriks = COL_EFF(main);

    readMatrix(&duakalidua, 2, 2);
    ...
    for (baris = 0; baris < barisMatriks - 1; baris++) {
        for (kolom = 0; kolom < kolomMatriks - 1; kolom++) {
            countPutar = 0;
            cocok = false;
            while (countPutar < 4) {
                if (!cocok && 
                    ELMT(main, baris, kolom) > ELMT(duakalidua, 0, 0) &&
                    ELMT(main, baris, kolom + 1) > ELMT(duakalidua, 0, 1) &&
                    ELMT(main, baris + 1, kolom) > ELMT(duakalidua, 1, 0) &&
                    ELMT(main, baris + 1, kolom + 1) > ELMT(duakalidua, 1, 1)) {
                    
                    cocok = true;
                    countCocok++;
                }
                putarMatriksKecil(&duakalidua);
                countPutar++;
            }
        }
    }
